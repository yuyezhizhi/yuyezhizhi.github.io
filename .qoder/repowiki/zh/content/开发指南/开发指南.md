...
- 运行时依赖
  ...
  - 参考路径：[package.json:11-22](file://package.json#L11-L22)
...
## 结论
本项目以 Vue 3 + Vite 为基础，结合路由懒加载与 Rollup 手动分包策略，在保证开发体验的同时兼顾了运行时性能。通过统一的数据与组件结构，开发者可以高效地扩展新的动画页面与功能模块。建议在新增页面时遵循现有模式（动态导入、数据集中管理、样式隔离），并在构建配置层面持续关注体积与缓存策略。
...
- 代码质量与 CI/CD
  ...
  - 参考路径：[package.json:6-10](file://package.json#L6-L10)，[README.md:50-67](file://README.md#L50-L67)