...
## 更新摘要
**变更内容**
- 新增自动播放和单轨循环功能，通过音频元素的 loop 属性实现无缝循环播放
- 将 loadAudio 函数改为异步实现，包含 Promise 等待机制确保音频完全加载后再播放
- 添加 Web Audio API 上下文自动恢复功能以解决浏览器自动播放策略问题
- 新增多个事件监听器实现完整的播放控制和状态管理
- 增强了音频加载的可靠性和用户体验
...
Visualizer["虚拟可视化页面<br/>AudioVisualizerPage.vue"]
end
Router --> Views
Views --> Wheel
Views --> Tower
...
Tower --> Uploader
Stereo --> Uploader
Visualizer --> |内部虚拟音频| 视觉渲染
```

**图表来源**
- [index.js:1-205](file://src/router/index.js#L1-L205)
- [AudioUploader.vue:1-530](file://src/components/AudioUploader.vue#L1-L530)
...
**图表来源**
- [AudioUploader.vue:136-178](file://src/components/AudioUploader.vue#L136-L178)
...
- [AudioVisualizerPage.vue:104-124](file://src/views/AudioVisualizerPage.vue#L104-L124)
...
- 关键实现要点
  ...
  - 加载音频：创建对象URL并赋给 audio 元素，随后初始化 AudioContext 与 AnalyserNode。
...
- 虚拟可视化页面（AudioVisualizerPage.vue）
  - 内部创建虚拟音频（oscillator + analyser），演示多种可视化模式。
  - 与真实音频上传组件不同，该页面不依赖外部音频，而是演示频谱绘制流程。
...
**章节来源**
- [AudioUploader.vue:95-98](file://src/components/AudioUploader.vue#L95-L98)
- [AudioUploader.vue:169-174](file://src/components/AudioUploader.vue#L169-L174)
...
#### 自动播放与循环播放
组件现已支持自动播放和单轨循环功能：
- 在音频元素中添加了 `loop` 属性，实现无缝循环播放
- `loadAudio` 函数改为异步实现，使用 Promise 等待音频完全加载后再播放
- 新增 `onEnded` 事件处理器，确保播放结束后能够正确处理状态
...
**章节来源**
- [AudioUploader.vue:80-90](file://src/components/AudioUploader.vue#L80-L90)
- [AudioUploader.vue:136-164](file://src/components/AudioUploader.vue#L136-L164)
- [AudioUploader.vue:228-257](file://src/components/AudioUploader.vue#L228-L257)